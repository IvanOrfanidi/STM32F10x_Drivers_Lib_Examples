...
inline void EntrCritSection(void)
{
    ...
    // avoid lost of one count in case of simultaneously calling from both
    // places
    ++CriticalSecCntr;
}
...
#define LongToBin(n) \
    (((n >> 21) & 0x80) | ((n >> 18) & 0x40) | ((n >> 15) & 0x20) | \
        ((n >> 12) & 0x10) | ((n >> 9) & 0x08) | ((n >> 6) & 0x04) | \
        ((n >> 3) & 0x02) | ((n)&0x01))
...
#define BIN16(b1, b2) ((__BIN(b1) << 8UL) + __BIN(b2))
#define BIN32(b1, b2, b3, b4) \
    ((((Int32U)__BIN(b1)) << 24UL) + (((Int32U)__BIN(b2)) << 16UL) + \
        (((Int32U)__BIN(b3)) << 8UL) + (Int32U)__BIN(b4))
